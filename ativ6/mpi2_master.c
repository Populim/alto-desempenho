...
int main(int argc,char* argv[]){
	...
	if (intercomm == MPI_COMM_NULL) {
		MPI_Comm_spawn( "mpi_slave", MPI_ARGV_NULL, N, MPI_INFO_NULL, 0, MPI_COMM_WORLD, &intercomm, errcodes );
	...
	printf("%p\n", B[0]);
	//MPI_Send(B[0], N*N, MPI_DOUBLE, 0, 0, intercomm);
	MPI_Bcast(B[0], N*N, MPI_DOUBLE, MPI_ROOT, intercomm);


	MPI_Scatter(A[0],N,MPI_DOUBLE,v,N,MPI_DOUBLE,MPI_ROOT,intercomm);
